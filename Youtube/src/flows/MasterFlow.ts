    ...
    try {
      ...
      if (!options.skipQuality) {
        ...
        qualityOutput = await qualityFlow({
          ...
          videoMetadata: productionOutput?.video ? {
            clipCount: productionOutput.video.clips.length,
            totalDuration: productionOutput.video.totalDuration,
            resolution: productionOutput.video.metadata.resolution,
            heroClipCount: productionOutput.video.metadata.heroClipCount,
          } : undefined,
          videoType: input.videoType,
          targetAudience: input.targetAudience,
      ...
      const totalCost = phases.research.cost + phases.production.cost + phases.quality.cost;
